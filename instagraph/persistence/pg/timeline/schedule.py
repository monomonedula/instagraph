...
class TimelineSchedule(
    FollowSchedule, metaclass=delegation_metaclass(delegate_attr="_schedule")
):
...
class NoFollowDuplications(
    ...
    def add_record(
        ...
        if self._pgsql.exec(
            ...
            raise HadBeenFollowing(
                f"User {user.id()} has history of following user {user_to_be_followed.id()}"
            )
        self._schedule.add_record(user, user_to_be_followed, tags, priority)
